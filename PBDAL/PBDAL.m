function [UU,X] = PBDAL(X,Y,d,numanchor,beta,gamma,ind)
...
while flag
    iter = iter + 1;
        ...
        %% optimize Z
    for v=1:numview
        temp_am{v} = (1+mu/2)*ones(1,numsample); 
        temp_Z = 0;
        for p = 1:numview
            if p == v
                continue;
        ...
        temp_PWX_ut{v} = (P{v}'*W{v}'*X{v}-0.5*beta*temp_Z+0.5*mu*(H{v} - J{v}/mu));
         for ii=1:numsample
            idx = 1:numanchor;
    ...
    term1 = 0;
    for v = 1:numview
        term1 = term1 + norm(W{v}'* X{v} - P{v} * (Z{v}),'fro')^2;
    end
    obj(iter) = term1;
